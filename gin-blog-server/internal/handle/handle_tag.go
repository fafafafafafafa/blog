...
func (*Tag) GetList(c *gin.Context) {
	...
	data, total, err := dao.GetTagList(GetDB(c), query.Page, query.Size, query.Keyword)
	if err != nil {
...
// @Security ApiKeyAuth
// @Router /tag [post]
func (*Tag) AddOrUpdate(c *gin.Context) {
	var form model.AddOrEditTagReq
	if err := c.ShouldBindJSON(&form); err != nil {
		ReturnError(c, g.ErrRequest, err)
		return
	}

	tag, err := dao.AddOrUpdateTag(GetDB(c), form.ID, form.Name)
	if err != nil {
		ReturnError(c, g.ErrDbOp, err)
		return
	}
...
func (*Tag) Delete(c *gin.Context) {
	...
	// 检查标签下面有没有文章
	count, err := dao.Count(db, &model.ArticleTag{}, "tag_id in ?", ids)
	if err != nil {
		ReturnError(c, g.ErrDbOp, err)
		return
	}
...
func (*Tag) GetOption(c *gin.Context) {
	list, err := dao.GetTagOption(GetDB(c))
	if err != nil {
		ReturnError(c, g.ErrDbOp, err)
		return
	}
