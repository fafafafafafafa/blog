...
func (*Message) UpdateReview(c *gin.Context) {
	...
	rows, err := dao.UpdateMessagesReview(GetDB(c), req.Ids, req.IsReview)
	if err != nil {
...
func (*Message) GetList(c *gin.Context) {
	var query model.MessageQuery
	if err := c.ShouldBindQuery(&query); err != nil {
		ReturnError(c, g.ErrRequest, err)
		return
	}

	data, total, err := dao.GetMessageList(GetDB(c), query.Page, query.Size, query.Nickname, query.IsReview)
	if err != nil {
		ReturnError(c, g.ErrDbOp, err)
		return
	}
	ReturnSuccess(c, model.PageResult[model.Message]{
		Total: total,
		List:  data,
		Size:  query.Size,
		Page:  query.Page,
...
func (*Message) Delete(c *gin.Context) {
	...
	rows, err := dao.DeleteMessages(GetDB(c), ids)
	if err != nil {
		ReturnError(c, g.ErrDbOp, err)
		return
	}
